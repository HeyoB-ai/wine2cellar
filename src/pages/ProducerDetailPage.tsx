  ...
  if (!producer) return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        <p className="text-cream-200/60 font-sans mb-4">Wijnhuis niet gevonden.</p>
        <Link to="/producers" className="btn-wine">Terug naar overzicht</Link>
      </div>
    </div>
  );
      ...
      <div className="relative h-96 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link to="/producers" className="inline-flex items-center gap-2 text-sm text-cream-300 hover:text-gold-400 mb-6 transition-colors">
            <ArrowLeft className="w-4 h-4" /> Terug naar wijnhuizen
          </Link>
          <div className="flex flex-wrap items-end gap-6">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-cream-300">
                ...
                <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-gold-400" />Opgericht {producer.founded}</span>
                {producer.hectares > 0 && <span>{producer.hectares} hectare</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-12">
            ...
            <section>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="card-dark p-5 text-center">
                  ...
                  <p className="text-sm text-cream-200/60 font-sans">Opgericht</p>
                </div>
                {producer.hectares > 0 && (
                  <div className="card-dark p-5 text-center">
                    <p className="font-display text-3xl font-bold text-gold-400 mb-1">{producer.hectares}</p>
                    <p className="text-sm text-cream-200/60 font-sans">Hectare wijngaard</p>
                  </div>
                )}
              </div>
            </section>
          ...
          <div className="space-y-6">
            ...
            {producer.awards.length > 0 && (
              <div className="card-dark p-6">
                <h3 className="font-display text-lg font-semibold text-cream-50 mb-4 flex items-center gap-2">
                  <Award className="w-5 h-5 text-gold-400" /> Onderscheidingen
                </h3>
            ...
            <div className="card-dark p-6">
              ...
              <table className="w-full text-sm font-sans">
                <tbody className="divide-y divide-noir-700">
                  {[
                    ['Regio', producer.region],
                    ['Land', producer.country],
                    ['Opgericht', producer.founded],
                    ['Wijnen', producer.wines.length],
                  ].map(([k, v]) => (
                    <tr key={String(k)}>
            ...
            <div className="card-dark p-6 bg-gradient-to-br from-wine-950/60 to-noir-800">
              <h3 className="font-display text-lg font-semibold text-cream-50 mb-3">Interesse?</h3>
              <p className="text-sm text-cream-200/60 font-sans mb-5">Neem contact op met dit wijnhuis of maak een account aan om te bestellen.</p>
              {producer.email && (
                <a href={`mailto:${producer.email}`} className="btn-gold w-full text-sm py-2.5 justify-center gap-2 mb-3">
                  <Mail className="w-4 h-4" /> Contact opnemen
                </a>
              )}
              {producer.website && (
                <a href={producer.website} target="_blank" rel="noopener noreferrer" className="btn-outline w-full text-sm py-2.5 justify-center gap-2">
                  <Globe className="w-4 h-4" /> Website bezoeken
                </a>
              )}
            </div>
          </div>
